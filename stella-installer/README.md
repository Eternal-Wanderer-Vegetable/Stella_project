...
七个页面（`src/` 下的静态 HTML，无前端构建步骤）：

| 页面 | 文件 | 作用 |
|---|---|---|
| 运行状态 | `run.html` | 启动/停止 Bot、实时日志、一键复制诊断信息 |
| 用量 | `usage.html` | 今日 token 消耗、按角色/端点/模型的分项、缓存命中率与降级状态 |
| 插件 | `plugins.html` | 装了哪些插件；点开看它的指令、工具，以及每个工具有没有被能力声明认领 |
| 配置 | `config.html` | 填写群号/连接/模型信息，写 `STELLA_HOME/.env`（模型列表可从 LM Studio 拉取） |
| 人格设定 | `persona.html` | 按共享空间编辑人格文件 |
| 环境自检 | `doctor.html` | 展示 `deploy doctor` 结果与修复提示 |
| 配置导入 | `migrate.html` | 预览并执行 `deploy migrate`（配置/记忆/人格/插件搬迁 + 数据库升级）。入口在「配置」页底部，不占顶栏：这是一次性动作 |

「用量」与「插件」各自独立成页而不是挂在「运行状态」下面：那两块都是**边看边翻**的清单
（插件页还要点开看详情），塞进以启停与日志为主的页面里会把主操作挤到屏幕外。三者读的
是同一次 `deploy status --json`，只是「运行状态」「用量」「插件」三页各取所需的那一段。

启动时（`index.html`）先跑一遍自检：未配置且**探测到旧版本安装**时进「配置导入」页，
