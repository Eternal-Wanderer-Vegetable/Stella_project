...
这条约束是贯彻到底的：需要联网的工具（查天气、查番剧）由插件自己发请求，而**判断该不该用工具、把结果压缩成一句话、以及把插件卡片渲染成图片，全部在本地完成**——没有任何一步把聊天内容交给第三方服务。
...
- **🔗 兼容 AstrBot 插件** —— 现成的 AstrBot 插件放进 `data/plugins/` 就能跑，不改源码。插件的卡片模板由**本地 Chromium** 出图，同样不出网

- **🔌 可扩展** —— Pipeline 前后置 Hook 机制，扩展目录自动加载

- **🧪 可验证** —— 1000+ 单元测试覆盖记忆晋升、跨用户隔离、两层归属、防编造护栏、路由降级与工具隔离；另有探针脚本对真实模型做回归验证（含一个专门复现「噪音环境下漏掉信息」的用例），以及一份量化四类路由错误的基准
...
1. 运行 `stop.bat` 停止程序
2. 把新版本解压到一个新目录
3. 双击 `Stella.exe`（或 `start.bat`）→ 确认「从旧版本导入」
...
最多的那个空间）。全新安装会把用户数据放在安装目录**同级**的 `StellaData/`，此后升级只需替换
程序目录；用 `python -m deploy paths` 可以查看数据目录的实际位置。
