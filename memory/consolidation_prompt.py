...
# ── 前缀缓存约束（2026-08-28 起，改模板前务必读）──────────────────
# **可变占位符必须排在模板最末尾。** 前缀缓存只能命中「第一处差异之前」的内容，
# 而本模板的固定部分（判断依据 + 空输出许可 + JSON 结构 + 要求 + 候选结构）占绝大多数。
# 此前 {current_summary} / {messages} 排在开头，导致可缓存前缀只有开头一句话，
# 后面全部固定指令每次都按全价重复计费——在线 API 下这是整合链路的主要成本来源。
#
# {types} / {usages} / {visibilities} 由模块常量填充、每次调用逐字相同，属于固定
# 前缀的一部分，可以留在上方；只有 {current_summary} / {messages} 每次都变，必须
# 留在数据分隔线之后。**新增固定文本一律加在分隔线以上。**
# tests/test_prompt_cache_prefix.py 会守住这条约束。
# ──────────────────────────────────────────────────────────

CONSOLIDATION_PROMPT = """你的任务：分析一段群聊记录，用 JSON 格式输出。
