...
- 发送者白名单只影响落库（本脚本不落库），但会标注候选归属是否在本窗口发送者
  集合内，供观察归属错误。
- ⚠️ **本脚本绕过入库路径**：窗口消息直接由 ``format_messages`` 拼成文本喂给模型，
  不经过 ``record_message`` / ``group_messages``。因此它能验证「模型能不能从消息里
  提取」，**不能**验证「消息有没有被记录进库」。2026-08-17 的缺陷（@ 消息因监听器
  优先级被拦截而从不入库）就发生在 probe 的盲区里——5 个正例全绿，线上却一条
  AT_MENTION 都没有。入库链路需靠 ``source_kind`` 分布统计与真实对话验证。
"""
