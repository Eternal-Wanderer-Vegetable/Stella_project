...
4. **与本地模型共享闸门**：默认与本地 LLM 同一个 LM Studio 实例，而一次检索要
   编码 20+ 条文本（query + 每条候选）；LM Studio 不限并发，不串行会出现间歇性
   变慢且难以定位，因此每次编码经 ``_maybe_gate()`` 走 ``MEMORY_EMBEDDING_GATE``
   指定的那道闸门（tag="embed"；默认 ``auto`` = 本地 LLM 槽，纯在线部署下等于不排队）。
