...
> `plugin-scaffold`（生成）与 `deploy capabilities`（[§14](#14-能力查询)）在本规范发布时**尚未实现**，分别排在落地方案的第 3 期与第 2 期。`plugin-check` 与三层声明加载已经可用。
...
**热重载**（尚未实现，落地方案第 4 期）：将由 `ASTRBOT_PLUGIN_HOT_RELOAD_ENABLED` 控制，**默认关闭**，通过群内管理员命令触发。设计上它能收回 handler、工具、能力声明与 `sys.modules` 里的模块，但**清不掉**这些：

- 裸 `asyncio.create_task()` 起的后台任务（这就是 [§4](#4-生命周期与加载时机) 要求走 `register_task` 的原因）
- 插件起的线程、注册的全局钩子、monkeypatch、第三方库的模块级状态
- 已经被别处持有的旧实例引用

所以热重载的定位是**调试便利，不等于重启**。怀疑状态不干净就重启。

## 14. 能力查询

> 本章描述的三个入口尚未实现（落地方案第 2 期与第 4 期）。当前查询能力清单的方式是读 `logs/boot_debug.log` 里的能力装配日志。
