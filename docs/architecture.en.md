...
**Hot reload** (`ASTRBOT_PLUGIN_HOT_RELOAD_ENABLED`, default off, not implemented yet) is a debugging convenience rather than a restart: it can reclaim handlers, tools, capability declarations and the modules in `sys.modules`, but not tasks started with a bare `asyncio.create_task()`, threads a plugin started, monkeypatches, or references to the old instance already held elsewhere. That is why the specification requires background tasks to go through `context.register_task`.
