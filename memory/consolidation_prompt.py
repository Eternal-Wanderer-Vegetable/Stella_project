...
# ── 空输出许可的依据（2026-08-11 实测，勿删勿改宽）───────────────────
# 20 个真实窗口 + 其中 5 个窗口人工基线交叉验证：人工判定应产出 0 条，模型输出 0 条，一致。
# 同时用合成正例（明确的显卡型号 / 饮食禁忌 / 职业）验证提取能力正常（3/3 命中）。
# 改动前平均 2.35 条/窗口、空输出率 5%，属于严重过度生成——20 个窗口凭空造出 31 条记忆。
# 结论：下方「memory_candidates 允许为空数组」与三条负例是修复的核心，
#       修改本模板前必须先跑 scripts/probe_consolidation.py --positive，
#       确保回归正例仍为 3/3；否则视为回退了已修复的过度生成缺陷。
# ──────────────────────────────────────────────────────────
